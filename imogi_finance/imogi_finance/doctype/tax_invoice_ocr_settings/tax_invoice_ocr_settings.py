    ...
    def validate(self):
        if self.ocr_provider == "Google Vision":
            if not self.google_vision_api_key:
                frappe.throw(_("Google Vision API Key is required when provider is Google Vision."))
            if not self.google_vision_endpoint:
                frappe.throw(_("Google Vision endpoint is required when provider is Google Vision."))
